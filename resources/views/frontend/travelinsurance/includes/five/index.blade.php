...
?>
    <div class="row">
        <div class="col-md-3">
           <div class="mt-3 side-bar filterdiv hidden-xs adjust-quoto coverage-mobile-view" style="background-color: white; border: 1px solid #c0c0c0; padding: 10px; margin-top: 10px; border-radius: 4px; ">
            <h2 style="margin: 0;font-size: 26px;font-weight: bold;">Adjust your quotes</h2>
              <h4 class="coverage" style="font-size: 21px;margin: 0;padding: 0;font-weight: bold;margin-bottom: 0;border: none;text-align: left;">Coverage: <input type="text" id="coverage_amount_for_slider" name="coverage_amount" value="$<?php echo $request->sum_insured2;?>" style="border:0; font-size:24px; color:#444; font-weight:bold;background: no-repeat;margin: 0;padding: 0;text-align: center;width: 150px;"></h4>
        ...
        @if (in_array('yes', $request->pre_existing))
            @include('frontend.travelinsurance.includes.five.yes')
        @else
            @include('frontend.travelinsurance.includes.five.yes')
            @include('frontend.travelinsurance.includes.five.no')
        @endif
